...
$multilingual_import_addon->set_post_saved_function('multilingual_addon_saved_post');
...
function multilingual_addon_import($post_id, $data, $import_options, $articleData, $logger) {
	...
	if ($multilingual_import_addon->can_update_meta('_multilingual_entry_slug', $import_options)) {
		update_post_meta($post_id, '_multilingual_entry_slug', $articleData['post_name']);
	}
...
function multilingual_addon_saved_post($post_id, $import, $logger) {
	global $multilingual_import_addon;

    $post_object = get_post($post_id);
    $language = get_post_meta($post_id, '_multilingual_entry_language', true);
    $common_id = get_post_meta($post_id, '_multilingual_entry_common_id', true);
    $slug = get_post_meta($post_id, '_multilingual_entry_slug', true);

    set_language($post_id, $language, $common_id, $slug);
}
